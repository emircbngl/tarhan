...
| T2 | pn1d **MMS uzamsal-mertebe** — gerçek `_poisson_newton` ve `_continuity_solve` operatörlerine ayrı ayrı MMS (Faz-1 "temiz mertebe" açık kalemini kapatır) | Langtangen MMS disiplini (rank-3 ile aynı ruh); manufactured smooth alan + sürekli-operatör kaynağı | Her operatör temiz O(h²) | ✅ SG-süreklilik ve nonlineer Poisson-Newton ikisi de **2.000** (6 grid 20→640, hata her h-yarılanmasında çeyreğe). İzole operatörler makine-hassasiyetine çözülür (Poisson-Newton tol=1e-13; SG doğrudan tridiag) ⇒ **Gummel gürültü tabanı atlatılır** — J-öz-yakınsamasını ~1e-4-bağılda sınırlayan dış-iterasyon artefaktıydı. Kaynak-işareti yük-taşıyan (ters işaret yakınsamaz — regresyon bekçili). KALAN: tam-kuplajlı SOLVE mertebesi (Gummel-bağlı) coupled-Newton ister | `layer0/semiconductor/test_pn1d_mms_order.py` |
