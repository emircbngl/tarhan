...
| 2D-2 | 2D diode I–V | DEVSIM `dio2_element_2d.py` | ideality 1.00±0.02, currents agree | **DONE** (with a measured lower bound on bias) — `models/pn2d.py` on DEVSIM's own mesh: I_n ratio 1.00000 at every bias, and over **0.3–0.5 V** I_p and total agree 0.99938→1.00000 with ideality **1.0120–1.0126** against DEVSIM's own 1.0121–1.0126. Biases ≤0.1 V were always excluded: currents ~1e-14 A where DEVSIM's own conservation is already 1.4e-2. **0.2 V is now excluded too, and that is a correction rather than a tightening**: the first cross-oracle CI run measured I_p there at 1.063 against this laptop's 1.011, and tightening `gummel_tol` on one machine moved it 1.011 → 1.011 → 1.0347 → 1.0273 for 1e-9 → 1e-14, non-monotonically, with iterations 3 → 27. A quantity that wanders as the criterion tightens was never converged; the earlier agreement was luck on one platform. The electron current is stable at 0.2 V and still checked to 1e-4. This is the first stage that tests transverse transport — current has to spread to reach the partial top contact. |
